...
def deploy_blessed_model_to_fastapi(
    ...
    for parent_model in models:
        ...
        for version in versions:
            print(
                f"Version {version.version_id}: Aliases = {list(version.version_aliases)}"
            )
    ...
    print(
        f"Copying model to deployment location: gs://{bucket_name}/{deployment_model_path}"
    )
    ...
    try:
        ...
        iam_request = SetIamPolicyRequest(
            resource=result.name, policy=policy  # This should be the full resource name
        )
