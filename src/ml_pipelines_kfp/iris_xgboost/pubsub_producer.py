...
class IrisDataPubSubProducer:
    ...
    def send_batch(self) -> None:
        ...
        for i in range(self.batch_size):
            ...
            try:
                future = self.publisher.publish(
                    self.topic_path, data=message_data, **attributes
                )
        ...
        logger.info(
            f"Sent batch of {len(batch_data)} samples to topic {self.topic_name}"
        )
            ...
            while True:
                ...
                if duration_minutes:
                    ...
                    if elapsed_minutes >= duration_minutes:
                        logger.info(
                            f"Production completed after {duration_minutes} minutes"
                        )
                        break
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate random Iris data for Pub/Sub"
    )
    parser.add_argument("--project-id", required=True, help="GCP Project ID")
    parser.add_argument(
        "--topic", default="iris-inference-data", help="Pub/Sub topic name"
    )
    parser.add_argument(
        "--batch-size", type=int, default=10, help="Number of samples per batch"
    )
    parser.add_argument(
