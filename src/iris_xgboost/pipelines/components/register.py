...
def upload_model(
    ...
    if isinstance(fs, gcsfs.GCSFileSystem):
        ...
        predict_schemata = aiplatform_v1.PredictSchemata(
            instance_schema_uri=instance_schema_uri if fs.exists(instance_schema_uri) else None,
            parameters_schema_uri=parameters_schema_uri if fs.exists(parameters_schema_uri) else None,
            prediction_schema_uri=prediction_schema_uri if fs.exists(prediction_schema_uri) else None,
        )
    else:
